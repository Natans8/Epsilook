...
- **It is the expansion's OWN ART, not text** — the inline mark warcraft.wiki.gg uses for the same job, **vendored under
  `site/img/expansions/`** (11 files, ~34 KB) and never hotlinked. The pack ships each rung's `major`, which indexes
  `CFG.expansionArt`, so the art is named in one place. `alt` is the short name, so a failed load still reads
  ...
  `expansionicon_*` for the first six expansions only, and its glue logos are "World of Warcraft" lockups that all read
  alike when shrunk.
