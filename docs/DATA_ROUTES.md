...
| **Wowhead's vocabulary**  | The label a selector prints, where the enum's name is not the word | Hand-fetched once from the spell filter page, committed under `build/enums/`, never a runtime route |
...
**Where a spell comes from is the one family with no client table at all.** A player is taught a spell by a trainer,
given it by a quest, or given an item that casts or teaches it, and that item is sold, dropped or held in something;
every one of those edges is a server world table. `spellSources` carries them as one row per spell and source, naming
the item where an item is what carries it, and `sourceKindNames` ships the five words. The two halves are read apart and
joined in `derive/sources.py`: a spell reaches an item, an item reaches whoever hands it over, and a loot line either
carries an item or draws from a pool that may draw from another, which is why the pools are followed two hops. ⚠ A few
sources of each kind are kept and the row carries how many there are: one recipe's generic learn spell is reached by
eighty thousand loot lines, and half the spells with a source have exactly one. Quests are named by `questNames`, and
the creatures, objects and items the rows point at join `referenceNames`.

**What a creature IS comes from the server, because the client barely says.** A morph and a summon both store a creature
id, and `creatures` carries the type it is billed as, its rank from normal up to world boss, and the faction template it
belongs to, which `referenceNames` already names; `creatureTypes` and `creatureRanks` ship the words. The columns are
`creature_template`'s, so they say what Epsilon spawns rather than what retail shows, the footing the object types and
the cast gates already stand on. The client's own `Creature` table carries the first two for 2,694 of the 27,314
creatures the default pack names, which is why the server dump is the source and not the fallback. The words ship keyed
by the value a creature carries rather than by the bit a mask sets, so no reader converts.
...
| **sounds**     | A kit, what it is for, whether it loops, its files | `soundRows`, `soundKitNames`, `soundTypes`                                                     |
...
against the id spaces, named, read by the Epsilon core or never read by it, a dummy's script argument, or an amount by
its shape. The core's reading is the strongest: wherever it tests a misc value bit by bit, builds a mask from it or
casts it to an enum, in a handler or in any walk of the aura's rows, that decides the slot, which is how the rating
auras read as combat-rating masks, the power-cost auras' second column as a power mask and the immunity auras as effect
and aura ids. They are generated off the data and the core rather than written by hand, a payload wins where it reads
the same value, and the payloads are the oracle `selectors_test.py` holds the typing to. A vocabulary a slot names ships
its words in `selectorVocabularies`, and a mask's vocabulary is keyed by its bits, so the words a mask names are the
bits it sets. A reference names its target in `referenceNames`, per build and per language: every creature, object,
item, spell, shapeshift form, screen effect, faction template, faction, skill line, enchantment and sound kit a
reference column of the build's own effect rows holds, and every item an effect creates, with the name the build already
reads for it and an empty one where it has none.

**The points column is typed the same way, and the numbers ship in `effectAmounts`.** Each value's `EffectBasePoints`
is an amount whose `into` is its unit and whose `scale` is what the stored number is divided by before it reads in it.
The units are the words the 9.2.7 descriptions actually write after an effect's points, mined rather than chosen:
percent, seconds, minutes, hours, yards, degrees, count, or empty for a plain amount of whatever the misc columns name.
A value is typed where the descriptions quote its points, bare or divided (`${$s1/1000} sec` is milliseconds read as
seconds), with one unit and scale in at least 70% of three or more quotes; percent where Wowhead's name marks it one;
a scripted argument where its misc columns already are one; an amount of undecided unit otherwise.

`effectAmounts` carries one row per spell and effect index: the amount resolved at the build's level cap, the way the
description prints it (the level is `meta.counts.effectAmountLevel`), both radii in yards, the facing in degrees, what
the amount gains per level and per spent resource, all in tenths; the chain and PvP multipliers in hundredths, a hundred
being unchanged; the spell and attack power coefficients and the variance (the amount rolls within half of it either
way) in thousandths; the value multiplier in hundredths, shipped only on the rows whose effect or aura the core reads it
for, a drain's or a leech's return per point, a burn's damage per point, a health funnel's, a mana shield's mana per
point absorbed and a jump's speed, because elsewhere the column holds values nobody reads, sentinels near 10^17 among
them; the mechanic and the created item. `GroupSizeBasePointsCoefficient` is not shipped: the Epsilon core loads it and
never reads it. An angle a spell's area takes is not an effect's: `spellCones` carries the cone's degrees and a line's
width in tenths, from `SpellTargetRestrictions`. The ids come from the rows and never from the names, because a language
column is joined to its ids by position.
...
Every field the build derives by declaration, printed off the registry by `python tools/routes.py` and guarded by
`tools/check.py`: a route changed in `build/pack/routes/flows.py` and not here fails the check rather than drifting.
What a field *is* stays in the family sections above; this is what each one reads and lands as.

<!-- routes: generated by tools/routes.py, do not edit -->

112 declared fields, in `build/pack/routes/flows.py`, in declaration order:

| field                | what it is                                                                                                                                                                                                                                                                                     | reads                                                                      | needs                                                                                                                                                                                                                                                                              | lands as                           |
|----------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------|
| factions             | the faction a spell sets                                                                                                                                                                                                                                                                       | FactionTemplate, Faction                                                   | effects.factions.named                                                                                                                                                                                                                                                             | rows                               |
| zone_music           | the music set a screen effect swaps in                                                                                                                                                                                                                                                         | ZoneMusic                                                                  | none                                                                                                                                                                                                                                                                               | records                            |
| ambiences            | the ambience a screen effect swaps in                                                                                                                                                                                                                                                          | SoundAmbience                                                              | none                                                                                                                                                                                                                                                                               | records                            |
| soundkit_files       | the files a sound kit plays                                                                                                                                                                                                                                                                    | SoundKitEntry                                                              | none                                                                                                                                                                                                                                                                               | sets                               |
| kit_types            | what each reached sound kit is for                                                                                                                                                                                                                                                             | SoundKit                                                                   | sound_type_names, used_kits                                                                                                                                                                                                                                                        | map                                |
| looping_kits         | the reached sound kits that loop until stopped                                                                                                                                                                                                                                                 | SoundKit                                                                   | used_kits                                                                                                                                                                                                                                                                          | ids                                |
| kit_names            | the names the pinned build gives the sound kits this pack reaches                                                                                                                                                                                                                              | SoundKitName                                                               | used_kits                                                                                                                                                                                                                                                                          | rows                               |
| animkit_anims        | the animations a kit segments                                                                                                                                                                                                                                                                  | AnimKitSegment                                                             | anim_ids                                                                                                                                                                                                                                                                           | sets                               |
| animkit_speeds       | the pace a kit plays each animation at                                                                                                                                                                                                                                                         | AnimKitSegment                                                             | none                                                                                                                                                                                                                                                                               | map                                |
| animkit_bonesets     | the body regions a kit's animation moves                                                                                                                                                                                                                                                       | AnimKitSegment, AnimKitConfigBoneSet, AnimKitBoneSet                       | none                                                                                                                                                                                                                                                                               | tree                               |
| anim_replacements    | the animation swaps a replacement set makes                                                                                                                                                                                                                                                    | AnimReplacement                                                            | anim_ids                                                                                                                                                                                                                                                                           | sets                               |
| keybinds             | the game functions a key override casts a spell from                                                                                                                                                                                                                                           | SpellKeyboundOverride                                                      | none                                                                                                                                                                                                                                                                               | records                            |
| motions              | the flight paths a missile can fly                                                                                                                                                                                                                                                             | SpellMissileMotion                                                         | none                                                                                                                                                                                                                                                                               | records                            |
| forms                | the shapeshift forms and the creatures they wear                                                                                                                                                                                                                                               | SpellShapeshiftForm                                                        | none                                                                                                                                                                                                                                                                               | rows, then assemble                |
| objects              | the gameobjects a spell spawns, named and modelled                                                                                                                                                                                                                                             | gameobject_template, GameObjectDisplayInfo                                 | none                                                                                                                                                                                                                                                                               | rows, then assemble                |
| mounts               | the displays a mount-granting spell puts you on                                                                                                                                                                                                                                                | Mount, MountXDisplay                                                       | creatures, names.names                                                                                                                                                                                                                                                             | rows, then assemble                |
| creature_names       | what the server calls each creature                                                                                                                                                                                                                                                            | creature_template                                                          | none                                                                                                                                                                                                                                                                               | map                                |
| creature_kinds       | what the server bills each creature as                                                                                                                                                                                                                                                         | creature_template                                                          | none                                                                                                                                                                                                                                                                               | rows                               |
| spell_trainers       | the creatures that teach each spell                                                                                                                                                                                                                                                            | creature_trainer, trainer_spell                                            | names.names                                                                                                                                                                                                                                                                        | sets                               |
| quest_rewards        | what each quest grants                                                                                                                                                                                                                                                                         | quest_template                                                             | none                                                                                                                                                                                                                                                                               | records                            |
| item_spells          | the spell each item casts or teaches                                                                                                                                                                                                                                                           | ItemEffect, ItemXItemEffect                                                | names.names                                                                                                                                                                                                                                                                        | sets                               |
| item_vendors         | the creatures that sell each item                                                                                                                                                                                                                                                              | npc_vendor                                                                 | none                                                                                                                                                                                                                                                                               | sets                               |
| creature_drops       | the creatures that drop each item                                                                                                                                                                                                                                                              | creature_loot_template                                                     | none                                                                                                                                                                                                                                                                               | rows                               |
| object_drops         | the gameobjects that hold each item                                                                                                                                                                                                                                                            | gameobject_loot_template                                                   | none                                                                                                                                                                                                                                                                               | rows                               |
| loot_references      | the items each pooled loot list holds                                                                                                                                                                                                                                                          | reference_loot_template                                                    | none                                                                                                                                                                                                                                                                               | rows                               |
| faction_names        | what every faction is called                                                                                                                                                                                                                                                                   | Faction                                                                    | none                                                                                                                                                                                                                                                                               | map                                |
| skill_names          | what every skill line is called                                                                                                                                                                                                                                                                | SkillLine                                                                  | none                                                                                                                                                                                                                                                                               | map                                |
| enchantment_names    | what every enchantment is called                                                                                                                                                                                                                                                               | SpellItemEnchantment                                                       | none                                                                                                                                                                                                                                                                               | map                                |
| creature_displays    | the displays a creature wears, by slot                                                                                                                                                                                                                                                         | creature_template_model, creature_template                                 | none                                                                                                                                                                                                                                                                               | sets, then ordered                 |
| display_models       | the model each creature display wears                                                                                                                                                                                                                                                          | CreatureDisplayInfo                                                        | none                                                                                                                                                                                                                                                                               | map                                |
| display_skins        | the textures a display paints its model with                                                                                                                                                                                                                                                   | CreatureDisplayInfo                                                        | none                                                                                                                                                                                                                                                                               | map, then nonzero                  |
| creature_model_files | each creature model's file                                                                                                                                                                                                                                                                     | CreatureModelData                                                          | none                                                                                                                                                                                                                                                                               | map                                |
| totem_displays       | the displays a totem wears, one per caster race                                                                                                                                                                                                                                                | spell_totem_model                                                          | none                                                                                                                                                                                                                                                                               | sets, then ordered                 |
| creatures            | composed of creature_displays, creature_model_files, creature_names, display_models, display_skins, totem_displays                                                                                                                                                                             | none                                                                       | creature_displays, creature_model_files, creature_names, display_models, display_skins, totem_displays                                                                                                                                                                             | CreatureModels                     |
| item_names           | the items a visual holds up, by name and quality                                                                                                                                                                                                                                               | ItemSearchName                                                             | none                                                                                                                                                                                                                                                                               | records                            |
| model_files          | the base file of each model resource                                                                                                                                                                                                                                                           | ModelFileData                                                              | none                                                                                                                                                                                                                                                                               | map                                |
| item_icons           | each item's appearances, the base look first                                                                                                                                                                                                                                                   | ItemModifiedAppearance, ItemAppearance                                     | none                                                                                                                                                                                                                                                                               | map                                |
| item_models          | each item's appearances, the base look first                                                                                                                                                                                                                                                   | ItemModifiedAppearance, ItemAppearance, ItemDisplayInfo                    | model_files                                                                                                                                                                                                                                                                        | map, then <lambda>                 |
| items                | composed of item_icons, item_models, item_names                                                                                                                                                                                                                                                | none                                                                       | item_icons, item_models, item_names                                                                                                                                                                                                                                                | ItemModels                         |
| effect_names         | what each effect name reaches: a file, an item, a display or a weapon slot                                                                                                                                                                                                                     | SpellVisualEffectName                                                      | named                                                                                                                                                                                                                                                                              | records, then without_placeholders |
| attachments          | the models a kit hangs on a unit, and where                                                                                                                                                                                                                                                    | SpellVisualKitModelAttach                                                  | creatures, effect_names, items                                                                                                                                                                                                                                                     | rows, then assemble                |
| area_models          | the ground models                                                                                                                                                                                                                                                                              | SpellVisualKitAreaModel                                                    | none                                                                                                                                                                                                                                                                               | map                                |
| emissions            | the ground model an emitter spawns copies of                                                                                                                                                                                                                                                   | SpellEffectEmission, SpellVisualKitAreaModel                               | none                                                                                                                                                                                                                                                                               | map                                |
| barrages             | the model a volley is made of, and where on the caster it spawns                                                                                                                                                                                                                               | BarrageEffect, SpellVisualEffectName                                       | none                                                                                                                                                                                                                                                                               | records                            |
| weapon_trails        | the trail models                                                                                                                                                                                                                                                                               | WeaponTrail                                                                | none                                                                                                                                                                                                                                                                               | map                                |
| missiles             | the projectiles a visual launches, from its base set and its raid set                                                                                                                                                                                                                          | SpellVisual, SpellVisualMissile                                            | effect_names                                                                                                                                                                                                                                                                       | rows, then assemble_missiles       |
| chains               | what a beam segment draws with                                                                                                                                                                                                                                                                 | SpellChainEffects                                                          | none                                                                                                                                                                                                                                                                               | records                            |
| beams                | the chain a beam draws, and its two ends                                                                                                                                                                                                                                                       | BeamEffect                                                                 | none                                                                                                                                                                                                                                                                               | records                            |
| dissolves            | the dissolve materials                                                                                                                                                                                                                                                                         | DissolveEffect, TextureBlendSet                                            | none                                                                                                                                                                                                                                                                               | records                            |
| glows                | the colour an edge glow paints                                                                                                                                                                                                                                                                 | EdgeGlowEffect                                                             | none                                                                                                                                                                                                                                                                               | records                            |
| glow_alphas          | how opaque an edge glow is                                                                                                                                                                                                                                                                     | EdgeGlowEffect                                                             | none                                                                                                                                                                                                                                                                               | map                                |
| shadowies            | the two colours of a ghost effect, and where it anchors                                                                                                                                                                                                                                        | ShadowyEffect                                                              | none                                                                                                                                                                                                                                                                               | records                            |
| screens              | what a screen effect does to the frame, the sky, the sound and the hour                                                                                                                                                                                                                        | ScreenEffect, FullScreenEffect, TextureBlendSet                            | none                                                                                                                                                                                                                                                                               | records                            |
| visual_screens       | the kit's route into a screen effect                                                                                                                                                                                                                                                           | SpellVisualScreenEffect                                                    | none                                                                                                                                                                                                                                                                               | map                                |
| fx                   | composed of beams, chains, dissolves, glow_alphas, glows, screens, shadowies, visual_screens                                                                                                                                                                                                   | none                                                                       | beams, chains, dissolves, glow_alphas, glows, screens, shadowies, visual_screens                                                                                                                                                                                                   | FxPayloads                         |
| proc_chains          | the character procedures                                                                                                                                                                                                                                                                       | SpellProceduralEffect                                                      | none                                                                                                                                                                                                                                                                               | map                                |
| proc_tints           | the character procedures                                                                                                                                                                                                                                                                       | SpellProceduralEffect                                                      | none                                                                                                                                                                                                                                                                               | records                            |
| proc_ghosts          | the character procedures                                                                                                                                                                                                                                                                       | SpellProceduralEffect                                                      | none                                                                                                                                                                                                                                                                               | map                                |
| proc_desats          | the character procedures                                                                                                                                                                                                                                                                       | SpellProceduralEffect                                                      | none                                                                                                                                                                                                                                                                               | map, then nonzero                  |
| proc_transps         | the character procedures                                                                                                                                                                                                                                                                       | SpellProceduralEffect                                                      | none                                                                                                                                                                                                                                                                               | map, then nonzero                  |
| proc_freezes         | the character procedures                                                                                                                                                                                                                                                                       | SpellProceduralEffect                                                      | none                                                                                                                                                                                                                                                                               | ids                                |
| proc_camos           | the character procedures                                                                                                                                                                                                                                                                       | SpellProceduralEffect                                                      | none                                                                                                                                                                                                                                                                               | ids                                |
| proc_ground          | the character procedures                                                                                                                                                                                                                                                                       | SpellProceduralEffect, SpellVisualKitAreaModel                             | none                                                                                                                                                                                                                                                                               | map                                |
| proc_trails          | the character procedures                                                                                                                                                                                                                                                                       | SpellProceduralEffect, WeaponTrail                                         | none                                                                                                                                                                                                                                                                               | map                                |
| proc_anims           | the character procedures                                                                                                                                                                                                                                                                       | SpellProceduralEffect                                                      | none                                                                                                                                                                                                                                                                               | records, then nonzero              |
| procs                | composed of proc_anims, proc_camos, proc_chains, proc_desats, proc_freezes, proc_ghosts, proc_ground, proc_tints, proc_trails, proc_transps                                                                                                                                                    | none                                                                       | proc_anims, proc_camos, proc_chains, proc_desats, proc_freezes, proc_ghosts, proc_ground, proc_tints, proc_trails, proc_transps                                                                                                                                                    | ProcEffects                        |
| kit_sounds           | what a kit's effect rows reach, by type                                                                                                                                                                                                                                                        | SpellVisualKitEffect                                                       | none                                                                                                                                                                                                                                                                               | sets                               |
| kit_visual_anims     | what a kit's effect rows reach, by type                                                                                                                                                                                                                                                        | SpellVisualKitEffect, SpellVisualAnim                                      | none                                                                                                                                                                                                                                                                               | sets                               |
| kit_animkits         | what a kit's effect rows reach, by type                                                                                                                                                                                                                                                        | SpellVisualKitEffect, SpellVisualAnim                                      | none                                                                                                                                                                                                                                                                               | sets                               |
| kit_dissolves        | what a kit's effect rows reach, by type                                                                                                                                                                                                                                                        | SpellVisualKitEffect                                                       | dissolves                                                                                                                                                                                                                                                                          | sets                               |
| kit_glows            | what a kit's effect rows reach, by type                                                                                                                                                                                                                                                        | SpellVisualKitEffect                                                       | glows                                                                                                                                                                                                                                                                              | sets                               |
| kit_shadowies        | what a kit's effect rows reach, by type                                                                                                                                                                                                                                                        | SpellVisualKitEffect                                                       | shadowies                                                                                                                                                                                                                                                                          | sets                               |
| kit_screens          | what a kit's effect rows reach, by type                                                                                                                                                                                                                                                        | SpellVisualKitEffect, SpellVisualScreenEffect                              | screens                                                                                                                                                                                                                                                                            | sets                               |
| kit_emissions        | what a kit's effect rows reach, by type                                                                                                                                                                                                                                                        | SpellVisualKitEffect                                                       | emissions                                                                                                                                                                                                                                                                          | sets                               |
| kit_barrages         | what a kit's effect rows reach, by type                                                                                                                                                                                                                                                        | SpellVisualKitEffect                                                       | barrages                                                                                                                                                                                                                                                                           | sets                               |
| kit_beams            | what a kit's effect rows reach, by type                                                                                                                                                                                                                                                        | SpellVisualKitEffect, BeamEffect, SpellChainEffects                        | chains                                                                                                                                                                                                                                                                             | sets                               |
| kit_procedures       | what a kit's effect rows reach, by type                                                                                                                                                                                                                                                        | SpellVisualKitEffect                                                       | none                                                                                                                                                                                                                                                                               | sets                               |
| kit_proc_chains      | what a kit's effect rows reach, by type                                                                                                                                                                                                                                                        | SpellVisualKitEffect, SpellChainEffects                                    | chains, proc_chains                                                                                                                                                                                                                                                                | sets                               |
| kits                 | composed of attachments, barrages, emissions, kit_animkits, kit_barrages, kit_beams, kit_dissolves, kit_emissions, kit_glows, kit_proc_chains, kit_procedures, kit_screens, kit_shadowies, kit_sounds, kit_visual_anims, procs                                                                 | none                                                                       | attachments, barrages, emissions, kit_animkits, kit_barrages, kit_beams, kit_dissolves, kit_emissions, kit_glows, kit_proc_chains, kit_procedures, kit_screens, kit_shadowies, kit_sounds, kit_visual_anims, procs                                                                 | assemble                           |
| spell_visuals        | the visuals a spell reaches, its redirects followed                                                                                                                                                                                                                                            | SpellXSpellVisual, SpellVisual                                             | none                                                                                                                                                                                                                                                                               | nested                             |
| visual_events        | what a visual plays, when, and for whom                                                                                                                                                                                                                                                        | SpellVisualEvent                                                           | none                                                                                                                                                                                                                                                                               | lists                              |
| visual_sounds        | the sound a visual's own animation events play                                                                                                                                                                                                                                                 | SpellVisual                                                                | none                                                                                                                                                                                                                                                                               | map                                |
| graph                | composed of spell_visuals, visual_events, visual_sounds                                                                                                                                                                                                                                        | none                                                                       | spell_visuals, visual_events, visual_sounds                                                                                                                                                                                                                                        | VisualGraph                        |
| seats                | each seat's attachment and what the rider and the vehicle animate                                                                                                                                                                                                                              | VehicleSeat                                                                | none                                                                                                                                                                                                                                                                               | records                            |
| vehicles             | each vehicle's seats, by slot                                                                                                                                                                                                                                                                  | Vehicle                                                                    | seats                                                                                                                                                                                                                                                                              | map, then assemble                 |
| names                | the spell list                                                                                                                                                                                                                                                                                 | SpellName, Spell                                                           | spell_subtexts                                                                                                                                                                                                                                                                     | map, then assemble                 |
| spell_subtexts       | the parenthetical rank or variant under a spell's name                                                                                                                                                                                                                                         | Spell                                                                      | none                                                                                                                                                                                                                                                                               | map                                |
| alt_names            | the names a spell can rename its target to                                                                                                                                                                                                                                                     | SpellOverrideName                                                          | effects.altnames                                                                                                                                                                                                                                                                   | map, then override_names           |
| spell_icons          | each spell's icon, from the row that has one                                                                                                                                                                                                                                                   | SpellMisc                                                                  | names.names                                                                                                                                                                                                                                                                        | map                                |
| props                | what SpellMisc says about a spell                                                                                                                                                                                                                                                              | SpellMisc                                                                  | names.names, spell_icons                                                                                                                                                                                                                                                           | rows, then assemble                |
| reach                | how far a spell reaches                                                                                                                                                                                                                                                                        | SpellMisc, SpellRange                                                      | names.names                                                                                                                                                                                                                                                                        | rows, then <lambda>                |
| channel_breaks       | the channels that movement cancels                                                                                                                                                                                                                                                             | SpellInterrupts                                                            | names.names                                                                                                                                                                                                                                                                        | ids                                |
| delivery             | how a spell is delivered: a cast time, a channel, or both                                                                                                                                                                                                                                      | SpellMisc, SpellCastTimes, SpellDuration                                   | channel_breaks, names.names                                                                                                                                                                                                                                                        | rows, then assemble_delivery       |
| aura_interrupts      | what removes a spell's aura                                                                                                                                                                                                                                                                    | SpellInterrupts                                                            | names.names                                                                                                                                                                                                                                                                        | map, then nonzero                  |
| summon_controls      | how a summoned creature is controlled                                                                                                                                                                                                                                                          | SummonProperties                                                           | none                                                                                                                                                                                                                                                                               | map                                |
| effects              | a spell's effects, and who each is aimed at, split into 26 landings                                                                                                                                                                                                                            | SpellEffect                                                                | keybinds, names.names, screens, summon_controls, target_bits                                                                                                                                                                                                                       | SpellEffectRows                    |
| spell_radii          | how far each radius reaches                                                                                                                                                                                                                                                                    | SpellRadius                                                                | none                                                                                                                                                                                                                                                                               | map                                |
| spell_cones          | the cone or line a spell's area takes                                                                                                                                                                                                                                                          | SpellTargetRestrictions                                                    | names.names                                                                                                                                                                                                                                                                        | rows                               |
| effect_values        | the numbers a template asks of each effect, split into 6 landings                                                                                                                                                                                                                              | SpellEffect (unrevised), SpellScaling (unrevised), SpellRadius (unrevised) | level, scaling                                                                                                                                                                                                                                                                     | EffectValues                       |
| spell_durations      | how long a spell lasts                                                                                                                                                                                                                                                                         | SpellMisc (unrevised), SpellDuration (unrevised)                           | none                                                                                                                                                                                                                                                                               | map                                |
| spell_ranges         | how far a spell's description says it reaches                                                                                                                                                                                                                                                  | SpellMisc (unrevised), SpellRange (unrevised)                              | none                                                                                                                                                                                                                                                                               | map                                |
| spell_stack_caps     | what an aura's options cap                                                                                                                                                                                                                                                                     | SpellAuraOptions (unrevised)                                               | none                                                                                                                                                                                                                                                                               | map                                |
| spell_charges        | what an aura's options cap                                                                                                                                                                                                                                                                     | SpellAuraOptions (unrevised)                                               | none                                                                                                                                                                                                                                                                               | map                                |
| spell_proc_chances   | what an aura's options cap                                                                                                                                                                                                                                                                     | SpellAuraOptions (unrevised)                                               | none                                                                                                                                                                                                                                                                               | map                                |
| spell_target_caps    | what a spell's targeting caps                                                                                                                                                                                                                                                                  | SpellTargetRestrictions (unrevised)                                        | none                                                                                                                                                                                                                                                                               | map                                |
| spell_target_levels  | what a spell's targeting caps                                                                                                                                                                                                                                                                  | SpellTargetRestrictions (unrevised)                                        | none                                                                                                                                                                                                                                                                               | map                                |
| values               | composed of effect_values.chain_targets, effect_values.misc_value, effect_values.period, effect_values.points, effect_values.radius, effect_values.variance, level, spell_charges, spell_durations, spell_proc_chances, spell_ranges, spell_stack_caps, spell_target_caps, spell_target_levels | none                                                                       | effect_values.chain_targets, effect_values.misc_value, effect_values.period, effect_values.points, effect_values.radius, effect_values.variance, level, spell_charges, spell_durations, spell_proc_chances, spell_ranges, spell_stack_caps, spell_target_caps, spell_target_levels | DescriptionValues                  |
| spell_descriptions   | what the tooltip says the cast does                                                                                                                                                                                                                                                            | Spell                                                                      | none                                                                                                                                                                                                                                                                               | map                                |
| spell_aura_texts     | what the buff says while it is on you                                                                                                                                                                                                                                                          | Spell                                                                      | none                                                                                                                                                                                                                                                                               | map                                |
| spell_variables      | the named variable bodies a description may interpolate                                                                                                                                                                                                                                        | SpellXDescriptionVariables, SpellDescriptionVariables                      | none                                                                                                                                                                                                                                                                               | map, then nonzero                  |
| area_parents         | each area's parent                                                                                                                                                                                                                                                                             | AreaTable                                                                  | none                                                                                                                                                                                                                                                                               | map                                |
| areas                | where a spell may be cast                                                                                                                                                                                                                                                                      | SpellCastingRequirements, AreaGroupMember, AreaTable                       | area_parents, zone_maps                                                                                                                                                                                                                                                            | rows, then assemble                |
...
| when you add                               | edit                                                                      |
|--------------------------------------------|---------------------------------------------------------------------------|
| A route from a visual, effect or spell row | One row in that family's table under [Read](#read), and `python tools/routes.py --write` |
