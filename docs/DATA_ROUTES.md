...
What every field reads and lands as, what every section is made of, and a diagram of every family are generated rather
than written: [the route reference](reference/routes.md), rewritten from the registries by every build of the default
pack and by `python tools/routes.py --write`, and held to the code by `tools/check.py`. This page says why the routes are
shaped the way they are; that one says what they are, and cannot drift from the code it describes. A note written under
a route's line in `build/pack/routes/flows.py` is the note the reference prints under it.
...
| when you add                               | edit                                                                              |
|--------------------------------------------|-----------------------------------------------------------------------------------|
| A route from a visual, effect or spell row | One row in that family's table under [Read](#read); the reference rewrites itself |
