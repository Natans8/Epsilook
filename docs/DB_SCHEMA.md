...
a screen effect through a kit rather than through an aura" used to mean writing a throwaway script that re-parsed
180 MB of CSV. Now it is a `SELECT`.
...
**DataGrip / PyCharm Professional** — `+ → Data Source → DuckDB`, point it at the file, let it download the driver.
The bundled driver is JDBC 1.3.1, so the builder pins the file to the **v1.0.0 storage format**; do not remove that pin
...
```sql
SELECT name FROM ref.enum_value WHERE enum_name = 'SpellEffectAura' AND value = 296;
-- SET_VEHICLE_ID
```
...
Knowledge that lived only as prose, shipped as joinable tables so raw ids are legible without a second window open.
Each cites where it came from:
...
WHERE  spell_id = 642;                          -- Divine Shield
...
```sql
SELECT path FROM ref.listfile WHERE fid = 3597252;
```

**Compare a table across every version at once** — the thing a single-schema database made painful:

```sql
SELECT 'v9_2_7' AS build, count(*) FROM v9_2_7."SpellEffect"
UNION ALL SELECT 'v10_2_7', count(*) FROM v10_2_7."SpellEffect"
UNION ALL SELECT 'v11_2_7', count(*) FROM v11_2_7."SpellEffect";
```
...
FROM   ref.relation
WHERE  schema_name = 'v9_2_7' AND to_table = 'SpellVisualEffectName' AND resolvable;
```
...
WHERE  schema_name = 'v9_2_7' AND comment IS NOT NULL AND table_name = 'SpellVisualKitEffect';
```

**Which columns are guesses, not confirmed?**

```sql
SELECT table_name, column_name FROM ref.column_info
WHERE  schema_name = 'v9_2_7' AND unverified ORDER BY 1, 2;
```
...
makes every integer a `BIGINT`. The builder applies the declaration — on 9.2.7 that is 40 `UTINYINT`, 78 `USMALLINT`,
78 `UINTEGER` and only 42 `BIGINT` columns. Where a declared type does not fit the data, that table falls back to
inference and **says so in the build log**; a silently widened column is schema drift nobody would notice.
