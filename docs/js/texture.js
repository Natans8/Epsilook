 ...
 */
window.EpsilookTexture = (() => {
    "use strict";

    const CFG = window.EpsilookConfig;
    const {$, el, targetClosest, fillTemplate} = window.EpsilookUtil;

    /** The pack textures are fetched against. Set by init(); see EpsilookTextureApi. */
    let versionId = () => "";

    /** @param {{versionId: () => string}} deps */
    function init(deps) {
    ...
    return {init, load, initHoverPreview, hideHoverPreview: hideTexPreview};
})();
