...
def _defaults(engine: LuaRuntime) -> Iterator[None]:
    """Settings are module-level Lua state and the engine is shared, so each
    test starts and ends at the declared defaults however it leaves off."""
    lua_function(engine, b"Epsilook.Config.Load")(None)
    yield
    lua_function(engine, b"Epsilook.Config.Load")(None)
...
def test_a_store_keeps_only_what_the_declarations_still_admit(engine: LuaRuntime) -> None:
    ...
    engine.execute(b"STORE = { page = 500, gone = 7 }")
    ...
    load(engine, None)
    engine.execute(b"STORE = { page = 35 }")
    assert load(engine, lua_table(engine, b"STORE"))[b"page"] == 35, "a value the declarations admit is kept"
    load(engine, None)
