...
lands; a provider in `tables/` may open what it left there. `fetch_sources`
brings every byte one build reads into the cache and returns a `Sources` saying
where each of them is; the other exports read the sources that are parsed rather
than merely located.
"""
