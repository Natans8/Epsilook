...
def display_rows(which: str, id_column: str) -> Callable[[Reads], SectionColumns]:
    ...
    def produce(reads: Reads) -> SectionColumns:
        ...
        return {
            id_column: [row.subject for row in rows],
...
def display_skins(reads: Reads) -> SectionColumns:
    ...
    return {"displayIds": [display for display, _fid in rows], "fids": [fid for _display, fid in rows]}
...
def morphs(reads: Reads) -> SectionColumns:
    ...
    return {"creatureIds": ids, "names": [reads.creatures.names.get(creature, "") for creature in ids]}
...
def shapeshifts(reads: Reads) -> SectionColumns:
    ...
    return {"ids": ids, "names": [reads.forms.names.get(form, "") for form in ids]}


def summons(reads: Reads) -> SectionColumns:
    """The name of each summoned creature."""
    ids = sorted({creature for summoned in reads.effects.summons.values() for creature, _control in summoned})
    return {"creatureIds": ids, "names": [reads.creatures.names.get(creature, "") for creature in ids]}
