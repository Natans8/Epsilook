...
MISSILE_MOTIONS = register(
    Section(
        ...
        produce=lambda reads: {
            ...
            "names": [reads.motions[motion].name if motion in reads.motions else "" for motion in reads.rows.motions],
            "projectiles": [
                reads.motions[motion].projectiles if motion in reads.motions else 0 for motion in reads.rows.motions
            ],
        },
...
ITEMS = register(
    Section(
        ...
        counts=(
            size("items", "ids"),
            Count("namedItems", lambda columns, _r: sum(1 for name in columns["names"] if name)),
        ),
        localizable=("names",),
    )
)

ITEM_ICON_NAMES = register(
    Section(
        name="itemIconNames",
