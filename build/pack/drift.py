...
# through Wrath; from Cataclysm the same ids carry SURVEY, CHANGE_RAID_MARKER
# and SHOW_CORPSE_LOOT, whose misc value is not an entry. WoWDBDefs names only
# the modern meaning, so a build's own enum cannot be asked which one it is on.
# A content fact: it follows the patch number on every client line.
RETIRED_SPAWN_OBJECT_EFFECTS = frozenset({105, 106, 107})
"""SUMMON_OBJECT_SLOT2 to SLOT4, on the builds where they still mean that."""

SPAWN_OBJECT_SLOTS_UNTIL = "4.0"
"""The first patch on which the three slots stopped spawning an object."""
