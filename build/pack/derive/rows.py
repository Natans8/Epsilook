...
def id_rows(ids: MaskedIds) -> list[tuple[int, int]]:
    ...
    """
    return sorted((spell, payload) for spell, payloads in ids.ids.items() for payload in payloads)
...
def spell_rows(
    per_vehicle: Mapping[int, set[int]], vehicles: list[tuple[int, int]], limit: int | None = None
) -> list[tuple[int, int]]:
...
def spell_role_rows(
    per_vehicle: Mapping[int, set[tuple[int, int]]], vehicles: list[tuple[int, int]], limit: int
...
def seat_rows(vehicle_ids: Sequence[int], seats: Mapping[int, Sequence[str]]) -> list[tuple[int, str]]:
...
def build_rows(
    ...
    return PackRows(
        ...
        items=sorted({row.ref for row in models if row.category == MODEL_CAT_ITEM and row.ref}),
        vehicles=vehicles,
...
def boneset_rows(
    bonesets: Mapping[int, Mapping[int, list[str]]], used: set[int]
    ...
    pool: dict[str, int] = {}
    rows = [
        (kit, anim, [pool.setdefault(name, len(pool)) for name in names])
        for kit in sorted(bonesets)
        if kit in used
