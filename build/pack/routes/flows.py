...
class Routes(Declarations):
    ...
    factions = flow("the faction a spell sets").read("FactionTemplate", c.ID, c.Faction, c.FactionGroup).narrow(
        c.ID, "effects.factions.named"
    ).join(c.Faction, "Faction", c.Name_lang) >> as_rows(
        FactionTemplateRow, c.ID, c.Faction, c.FactionGroup, word(c.Name_lang), sort=True
    )
    """The template each aura sets, resolved to the faction's name and group."""

    zone_music = flow("the music set a screen effect swaps in").read(
        "ZoneMusic", c.ID, c.SetName, c.Sounds_0, c.Sounds_1
    ) >> as_records(c.ID, ZoneMusic, word(c.SetName), c.Sounds_0, c.Sounds_1)

    ambiences = flow("the ambience a screen effect swaps in").read(
        "SoundAmbience", c.ID, c.AmbienceID_0, c.AmbienceID_1
    ) >> as_records(c.ID, Ambience, c.AmbienceID_0, c.AmbienceID_1)

    soundkit_files = flow("the files a sound kit plays").read("SoundKitEntry", c.SoundKitID, c.FileDataID).where(
        (c.SoundKitID != 0) & (c.FileDataID != 0)
    ) >> as_sets(c.SoundKitID, c.FileDataID)

    kit_types = flow("what each reached sound kit is for").read("SoundKit", c.ID, c.SoundType).narrow(
        c.ID, "used_kits"
    ).narrow(c.SoundType, "sound_type_names", "the types the vendored enum names") >> as_map(c.ID, c.SoundType)

    kit_names = flow("the names the pinned build gives the sound kits this pack reaches").read(
        "SoundKitName", c.ID, c.Name, source="pinned"
    ).narrow(c.ID, "used_kits").map("named", ~c.Name.is_empty()).where(c.named == 1) >> as_rows(
        lambda kit, name: (kit, name), c.ID, word(c.Name), sort=True
    )
    """Kits added after the pinned build have no name anywhere and stay unnamed."""

    animkit_anims = flow("the animations a kit segments").read("AnimKitSegment", c.ParentAnimKitID, c.AnimID).where(
        c.ParentAnimKitID != 0
    ).narrow(c.AnimID, "anim_ids") >> as_sets(c.ParentAnimKitID, c.AnimID)

    animkit_speeds = flow("the pace a kit plays each animation at").read(
        "AnimKitSegment", c.ParentAnimKitID, c.AnimID, c.Speed
    ).where(c.ParentAnimKitID != 0) >> as_map(
    ...
    animkit_bonesets = flow("the body regions a kit's animation moves").read(
        "AnimKitSegment", c.ParentAnimKitID, c.AnimID, c.AnimKitConfigID
    ).join(
        c.AnimKitConfigID, "AnimKitConfigBoneSet", c.AnimKitBoneSetID, by="ParentAnimKitConfigID", inner=True, many=True
    ).join(c.AnimKitBoneSetID, "AnimKitBoneSet", c.Name, inner=True).where(
        ~c.Name.is_empty() & (c.Name != "Full Body")
    ...
    anim_replacements = flow("the animation swaps a replacement set makes").read(
        "AnimReplacement", c.ParentAnimReplacementSetID, c.SrcAnimID, c.DstAnimID
    ).where(c.ParentAnimReplacementSetID != 0).narrow(c.SrcAnimID, "anim_ids").narrow(
        c.DstAnimID, "anim_ids"
    ) >> as_sets(c.ParentAnimReplacementSetID, c.SrcAnimID, c.DstAnimID)

    keybinds = flow("the game functions a key override casts a spell from").read(
    ...
    motions = flow("the flight paths a missile can fly").read("SpellMissileMotion", c.ID, c.Name, c.MissileCount).where(
        ~c.Name.is_empty()
    ...
    creature_names = flow("what the server calls each creature").read(
        "creature_template", c.entry, c.name, source="world"
    ...
    display_models = flow("the model each creature display wears").read(
        "CreatureDisplayInfo", c.ID, c.ModelID
    ...
    creature_model_files = flow("each creature model's file").read("CreatureModelData", c.ID, c.FileDataID) >> as_map(
        c.ID, c.FileDataID
    )
    ...
    item_names = flow("the items a visual holds up, by name and quality").read(
        "ItemSearchName", c.ID, c.Display_lang, c.OverallQualityID
    ).where(~c.Display_lang.is_empty()) >> as_records(c.ID, ItemName, text(c.Display_lang), c.OverallQualityID)

    model_files = flow("the base file of each model resource").read(
        "ModelFileData", c.FileDataID, c.ModelResourcesID
    ).where((c.FileDataID != 0) & (c.ModelResourcesID != 0)) >> as_map(c.ModelResourcesID, c.FileDataID, reduce=min)
    ...
    area_models = flow("the ground models").read("SpellVisualKitAreaModel", c.ID, c.ModelFileDataID) >> as_map(
        c.ID, c.ModelFileDataID
    )

    emissions = flow("the ground model an emitter spawns copies of").read(
        "SpellEffectEmission", c.ID, c.AreaModelID
    ).join(c.AreaModelID, "SpellVisualKitAreaModel", c.ModelFileDataID, inner=True).where(
        c.ModelFileDataID != 0
    ) >> as_map(c.ID, typed(c.ModelFileDataID, ground_model))

    barrages = flow("the model a volley is made of, and where on the caster it spawns").read(
        "BarrageEffect", c.ID, c.SpellVisualEffectNameID, c.AttachmentPoint
    ).join(c.SpellVisualEffectNameID, "SpellVisualEffectName", c.ModelFileDataID, inner=True).where(
        c.ModelFileDataID != 0
    ...
    chains = flow("what a beam segment draws with").read(
        "SpellChainEffects",
        c.ID,
        c.Red,
        c.Green,
    ...
    beams = flow("the chain a beam draws, and its two ends").read(
        "BeamEffect", c.ID, c.BeamID, c.SourceAttachID, c.DestAttachID
    ) >> as_records(c.ID, Beam, c.BeamID, c.SourceAttachID, c.DestAttachID)

    dissolves = flow("the dissolve materials").read(
        "DissolveEffect", c.ID, c.TextureBlendSetID, c.Duration, c.AttachID
    ).join(c.TextureBlendSetID, "TextureBlendSet", c.TextureFileDataID[:]) >> as_records(
        c.ID, Dissolve, typed(c.Duration, seconds), typed(c.TextureFileDataID[:], ids_of), c.AttachID
    )

    glows = flow("the colour an edge glow paints").read(
        "EdgeGlowEffect", c.ID, c.GlowRed, c.GlowGreen, c.GlowBlue
    ) >> as_records(
        c.ID,
    ...
    glow_alphas = flow("how opaque an edge glow is").read("EdgeGlowEffect", c.ID, c.GlowAlpha) >> as_map(
        c.ID, typed(c.GlowAlpha, channel)
    )

    shadowies = flow("the two colours of a ghost effect, and where it anchors").read(
    ...
    screens = flow("what a screen effect does to the frame, the sky, the sound and the hour").read(
        "ScreenEffect",
        c.ID,
        c.Name,
        c.Param_0,
    ...
    visual_screens = flow("the kit's route into a screen effect").read(
        "SpellVisualScreenEffect", c.ID, c.ScreenEffectID
    ...
    proc_chains = procedures.where(c.Type.among(PROC_TYPES_CHAIN)) >> as_map(c.ID, c.Value_0)

    proc_tints = procedures.where(c.Type.among((PROC_TYPE_TINT, PROC_TYPE_TINT_MAT))) >> as_records(
        c.ID, tint, c.Type, c.Value_0, c.Value_3
    )
    """The payload column differs per Type, and a colourless tint folds in as black."""

    proc_ghosts = procedures.where((c.Type == PROC_TYPE_GHOST_MAT) & (c.Value_3 != 0)) >> as_map(
        c.ID, typed(c.Value_3, rgb_of)
    )
    """A colourless ghost has nothing to show and is dropped."""

    proc_desats = (procedures.where(c.Type == PROC_TYPE_DESATURATE) >> as_map(c.ID, typed(c.Value_2, percent))).then(
        nonzero
    )

    proc_transps = (procedures.where(c.Type == PROC_TYPE_TRANSPARENCY) >> as_map(c.ID, typed(c.Value_0, percent))).then(
        nonzero
    )
    """A percentage of zero would render as a claim that something happened."""

    proc_freezes = procedures.where(c.Type == PROC_TYPE_FREEZE) >> as_ids(c.ID)

    proc_camos = procedures.where(c.Type == PROC_TYPE_CAMO) >> as_ids(c.ID)

    proc_ground = procedures.where(c.Type == PROC_TYPE_AREAMODEL).join(
        c.Value_0, "SpellVisualKitAreaModel", c.ModelFileDataID, inner=True
    ).where(c.ModelFileDataID != 0) >> as_map(c.ID, typed(c.ModelFileDataID, ground_model))

    proc_trails = procedures.where(c.Type == PROC_TYPE_WEAPONTRAIL).join(
        c.Value_0, "WeaponTrail", c.FileDataID, inner=True
    ).where(c.FileDataID != 0) >> as_map(c.ID, typed(c.FileDataID, trail_model))
    ...
    kit_visual_anims = kit_anim_rows.explode(c.InitialAnimID, c.LoopAnimID, into="anim").where(c.anim > 0) >> as_sets(
        c.ParentSpellVisualKitID, c.anim
    )
    ...
    kit_dissolves = kit_effects.where(c.EffectType == EFFECT_TYPE_DISSOLVE).narrow(c.Effect, "dissolves") >> as_sets(
        c.ParentSpellVisualKitID, c.Effect
    )

    kit_glows = kit_effects.where(c.EffectType == EFFECT_TYPE_EDGE_GLOW).narrow(c.Effect, "glows") >> as_sets(
        c.ParentSpellVisualKitID, c.Effect
    )

    kit_shadowies = kit_effects.where(c.EffectType == EFFECT_TYPE_SHADOWY).narrow(c.Effect, "shadowies") >> as_sets(
        c.ParentSpellVisualKitID, c.Effect
    )
    """A row pointing at a payload this build lacks is dropped rather than an error."""

    kit_screens = kit_effects.where(c.EffectType == EFFECT_TYPE_SCREEN).join(
        c.Effect, "SpellVisualScreenEffect", c.ScreenEffectID, inner=True
    ).narrow(c.ScreenEffectID, "screens") >> as_sets(c.ParentSpellVisualKitID, c.ScreenEffectID)

    kit_emissions = kit_effects.where(c.EffectType == EFFECT_TYPE_EMISSION).narrow(c.Effect, "emissions") >> as_sets(
        c.ParentSpellVisualKitID, c.Effect
    )

    kit_barrages = kit_effects.where(c.EffectType == EFFECT_TYPE_BARRAGE).narrow(c.Effect, "barrages") >> as_sets(
        c.ParentSpellVisualKitID, c.Effect
    )

    kit_beams = kit_effects.where(c.EffectType == EFFECT_TYPE_BEAM).join(
        c.Effect, "BeamEffect", c.BeamID, c.SourceAttachID, c.DestAttachID, inner=True
    ).expand(c.BeamID, "SpellChainEffects", {"SpellChainEffectID_*": 0}, into="chain", bits="hops").narrow(
        c.chain, "chains"
    ...
    kit_proc_chains = kit_effects.where(c.EffectType == EFFECT_TYPE_PROC).lookup(
        c.Effect, "proc_chains", into="seed"
    ).expand(c.seed, "SpellChainEffects", {"SpellChainEffectID_*": 0}, into="chain", bits="hops").narrow(
        c.chain, "chains"
    ...
    spell_visuals = flow("the visuals a spell reaches, its redirects followed").read(
        "SpellXSpellVisual", c.SpellID, c.SpellVisualID
    ).where((c.SpellID != 0) & (c.SpellVisualID != 0)).expand(
        c.SpellVisualID, "SpellVisual", VISUAL_REDIRECTS, into="visual", bits="reached"
    ...
    visual_events = flow("what a visual plays, when, and for whom").read(
        "SpellVisualEvent", c.SpellVisualID, c.SpellVisualKitID, c.TargetType, c.StartEvent
    ).where((c.SpellVisualID != 0) & (c.SpellVisualKitID != 0)) >> as_lists(
        c.SpellVisualID, c.SpellVisualKitID, c.StartEvent, typed(c.TargetType, target_bit), record=KitEvent
    )
    """Distinct, in table order, and kept per event rather than folded per kit:
    the phase is what the pack ships."""

    visual_sounds = flow("the sound a visual's own animation events play").read(
        "SpellVisual", c.ID, c.AnimEventSoundID
    ).where(c.AnimEventSoundID != 0) >> as_map(c.ID, c.AnimEventSoundID)

    graph = compose(VisualGraph)

    # The vehicles.

    seats = flow("each seat's attachment and what the rider and the vehicle animate").read(
    ...
    spell_subtexts = flow("the parenthetical rank or variant under a spell's name").read(
        "Spell", c.ID, c.NameSubtext_lang
    ).where(~c.NameSubtext_lang.is_empty()) >> as_map(c.ID, text(c.NameSubtext_lang))
    ...
    spell_icons = flow("each spell's icon, from the row that has one").read(
        "SpellMisc", c.SpellID, c.DifficultyID, c.SpellIconFileDataID
    ).narrow(c.SpellID, "names.names").where(c.SpellIconFileDataID != 0).prefer(c.SpellID, base=BASE) >> as_map(
        c.SpellID, c.SpellIconFileDataID
    )
    ...
    channel_breaks = flow("the channels that movement cancels").read(
        "SpellInterrupts", c.SpellID, c.DifficultyID, c.ChannelInterruptFlags[:], optional=True
    ).narrow(c.SpellID, "names.names").prefer(c.SpellID, base=BASE).where(
        c.ChannelInterruptFlags[:].bit(MOVING_BIT)
    ) >> as_ids(c.SpellID)
    ...
    summon_controls = flow("how a summoned creature is controlled").read("SummonProperties", c.ID, c.Control) >> as_map(
        c.ID, c.Control
    )
    ...
    spell_durations = flow("how long a spell lasts").read(
        "SpellMisc", c.SpellID, c.DifficultyID, c.DurationIndex, source="base"
    ).where(BASE).join(c.DurationIndex, "SpellDuration", c.Duration, inner=True, source="base") >> as_map(
        c.SpellID, c.Duration
    )

    spell_ranges = flow("how far a spell's description says it reaches").read(
        "SpellMisc", c.SpellID, c.DifficultyID, c.RangeIndex, source="base"
    ).where(BASE).join(c.RangeIndex, "SpellRange", c.RangeMax_0, inner=True, source="base").map(
        "distance", coalesce(c.RangeMax_0, digits=1)
    ).where(c.distance != 0) >> as_map(c.SpellID, real(c.distance))
    ...
    spell_charges = aura_caps.where(c.ProcCharges != 0) >> as_map(c.SpellID, c.ProcCharges)
    ...
    spell_descriptions = flow("what the tooltip says the cast does").read("Spell", c.ID, c.Description_lang).where(
        ~c.Description_lang.is_empty()
    ...
    spell_aura_texts = flow("what the buff says while it is on you").read("Spell", c.ID, c.AuraDescription_lang).where(
        ~c.AuraDescription_lang.is_empty()
    ...
    area_parents = flow("each area's parent").read("AreaTable", c.ID, c.ParentAreaID) >> as_map(c.ID, c.ParentAreaID)
