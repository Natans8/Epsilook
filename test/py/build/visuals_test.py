...
def test_the_phase_words_index_by_event_and_leave_the_unnamed_blank() -> None:
    ...
    assert words[0] == "none"
    # The client's enum runs to 24 and names 14 of them; the rest are real
    # events the data does carry, so they index an entry rather than nothing.
    # The pack's own launch phase sits one past the client's last value.
    assert len(words) == 26
    assert all(words[event] for event in range(14)) and not any(words[event] for event in range(14, 25))
    assert words[25] == "launch"
