...
from pack.sources.listfile import LISTFILE_ASSET, LISTFILE_DIR, SUPPLEMENT, SUPPLEMENT_FLOOR
...
def listfile(tmp_path: Path, text: str = LINES, name: str = "listfile.csv") -> ListfileTables:
    path = tmp_path / name
...
def test_a_wanted_id_the_listfile_does_not_name_is_absent(tmp_path: Path) -> None:
    """Rather than present and empty, so a caller cannot mistake an unnamed
    asset for one named the empty string."""
    assert resolve_paths(listfile(tmp_path), {1, 4242}) == {1: "Interface/Cinematics/Logo_800.avi"}
...
def test_the_provider_serves_one_table_and_names_its_columns(tmp_path: Path) -> None:
...
def test_the_provider_projects_columns_in_the_order_asked_for(tmp_path: Path) -> None:
    ...
    tables = listfile(tmp_path)
    assert next(iter(tables.rows(TABLE, [PATH, ID]))) == ("Interface/Cinematics/Logo_800.avi", "1")
    assert next(iter(tables.rows(TABLE, [ID]))) == ("1",)


def test_the_provider_hands_back_the_source_text_and_the_route_trims_it(tmp_path: Path) -> None:
    ...
    assert next(iter(tables.rows(TABLE, [ID, PATH]))) == ("7", "  Interface/Padded.blp  ")
...
def _supplemented(tmp_path: Path, supplement: str) -> OverlaidTables:
    ...
    return OverlaidTables(
        ...
        overlays={TABLE: Overlay(TABLE, {ID: ID, PATH: PATH}, key=ID, admits=above(SUPPLEMENT_FLOOR))},
        source=listfile(tmp_path, supplement, name="supplement.csv"),
    )


def test_a_supplement_names_what_the_community_listfile_cannot(tmp_path: Path) -> None:
...
def test_a_supplement_may_not_rename_an_asset_the_base_already_names(tmp_path: Path) -> None:
...
def test_no_community_id_reaches_the_supplement_floor() -> None:
    ...
    ceiling = max(int(fid) for fid, _path in ListfileTables(cached).rows(TABLE, [ID, PATH]))
