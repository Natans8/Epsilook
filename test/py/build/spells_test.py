...
def read_spell_properties(source: Tables, spells: Container[int]) -> SpellProperties:
    ...
    icons = Routes.spell_icons.run(source, needs={"names.names": spells})
    return Routes.props.run(source, needs={"names.names": spells, "spell_icons": icons})
...
"""
    ...
    assert spells.range_index == {100: 5, 200: 0}
    assert spells.attribute_words == {100: (8,), 200: (16,)}


def test_a_spell_with_no_base_row_keeps_the_first_it_has(tables: BuildTables) -> None:
    spells = read_spell_properties(
        tables(
            SpellMisc=f"""\
{HEADER}
...
def test_the_index_columns_follow_the_same_rule_as_the_school(tables: BuildTables) -> None:
    """Zero is an answer for all three -- it means the spell names no such row --
    so the base row's zero must displace a difficulty row's number."""
    spells = read_spell_properties(
        tables(
            SpellMisc=f"""\
{HEADER}
...
"""
    ...
    assert spells.range_index == {100: 0}
...
def test_every_attribute_column_the_build_exports_is_read(tables: BuildTables) -> None:
