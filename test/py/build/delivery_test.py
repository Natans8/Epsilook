...
from pack.routes.delivery import (BREAKS_ON_MOVE, CHANNELLED,
                                  read_spell_delivery)
...
def source(tables: BuildTables, interrupts: str | None = None) -> CsvTables:
    ...
    if interrupts is None:
        return tables(SpellCastTimes=CAST_TIMES, SpellDuration=DURATIONS,
                      absent={"SpellInterrupts": "predates the table"})
    return tables(SpellCastTimes=CAST_TIMES, SpellDuration=DURATIONS,
                  SpellInterrupts=interrupts)


def test_a_cast_and_a_channel_both_survive_on_one_spell(
    ...
    rows = read_spell_delivery(source(tables),
                               properties({100: (3, 5, CHANNEL_WORD)}))
    assert len(rows) == 1
...
def test_a_channel_may_carry_no_duration_row(tables: BuildTables) -> None:
    ...
    rows = read_spell_delivery(source(tables),
                               properties({100: (0, 0, CHANNEL_WORD)}))
    assert (rows[0].duration_ms, rows[0].flags) == (0, CHANNELLED)


def test_the_self_channelled_flag_is_still_a_channel(
        tables: BuildTables) -> None:
    rows = read_spell_delivery(source(tables),
                               properties({100: (0, 5, SELF_CHANNEL_WORD)}))
    assert rows[0].flags & CHANNELLED
...
def test_the_base_interrupt_row_overrides_a_difficulty_one(
        tables: BuildTables) -> None:
    rows = read_spell_delivery(
        source(tables, f"SpellID,DifficultyID,ChannelInterruptFlags_0\n"
                       f"100,23,{MOVING}\n100,0,0\n"),
...
def test_a_build_without_the_interrupts_table_keeps_the_rest(
        tables: BuildTables) -> None:
...
def test_movement_is_read_from_the_channel_column_only(
        tables: BuildTables) -> None:
...
def test_the_rows_come_back_sorted(tables: BuildTables) -> None:
    rows = read_spell_delivery(
        source(tables), properties({300: (3, 0, 0), 100: (3, 0, 0)}))
