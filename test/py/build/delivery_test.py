...
def deliver(
    tables: BuildTables, spells: dict[int, tuple[int, int, int]], interrupts: str | None = None
) -> list[Delivery]:
    """The delivery rows of a build holding just these spells.

    Args:
        tables: the fixture source.
        spells: spell id to its `(cast index, duration index, channel word)`,
            which becomes that spell's base `SpellMisc` row.
        interrupts: a `SpellInterrupts` table, or none for a build predating it.
    """
    misc = "SpellID,DifficultyID,CastingTimeIndex,DurationIndex,Attributes_0,Attributes_1\n" + "".join(
        f"{spell},0,{cast},{duration},0,{word}\n" for spell, (cast, duration, word) in spells.items()
    )
    held = {"SpellCastTimes": CAST_TIMES, "SpellDuration": DURATIONS, "SpellMisc": misc}
    if interrupts is None:
        source = tables(**held, absent={"SpellInterrupts": "predates the table"})
    else:
...
def test_a_cast_and_a_channel_both_survive_on_one_spell(tables: BuildTables) -> None:
    ...
    rows = deliver(tables, {100: (3, 5, CHANNEL_WORD)})
    assert len(rows) == 1
...
def test_a_spell_with_neither_is_omitted(tables: BuildTables) -> None:
    ...
    assert deliver(tables, {100: (0, 0, 0)}) == []
...
def test_a_channel_may_carry_no_duration_row(tables: BuildTables) -> None:
    ...
    rows = deliver(tables, {100: (0, 0, CHANNEL_WORD)})
...
def test_the_self_channelled_flag_is_still_a_channel(tables: BuildTables) -> None:
    rows = deliver(tables, {100: (0, 5, SELF_CHANNEL_WORD)})
    assert rows[0].flags & CHANNELLED


def test_a_channel_broken_by_movement_says_so(tables: BuildTables) -> None:
    rows = deliver(
        tables, {100: (0, 5, CHANNEL_WORD)}, f"SpellID,DifficultyID,ChannelInterruptFlags_0\n100,0,{MOVING}\n"
    )
...
def test_movement_is_read_from_the_channel_column_only(tables: BuildTables) -> None:
    ...
    rows = deliver(tables, {100: (0, 5, CHANNEL_WORD)}, "SpellID,DifficultyID,ChannelInterruptFlags_0\n100,0,1\n")
    assert not rows[0].flags & BREAKS_ON_MOVE


def test_the_rows_come_back_sorted(tables: BuildTables) -> None:
    rows = deliver(tables, {300: (3, 0, 0), 100: (3, 0, 0)})
