...
class Table:
    ...
    def blob(self) -> bytes:
        ...
        head = (
            _HEADER.size
            + _SECTION.size * len(self.sections)
        ...
        for section in self.sections:
            ...
            if section.relations:
                declared = len(section.relations) if section.declared_relations is None else section.declared_relations
                relations = struct.pack("<3I", declared, 0, 0)
                for foreign, index in section.relations:
                    relations += struct.pack("<II", foreign, index)
            copies = b"".join(struct.pack("<II", new_id, copied) for new_id, copied in section.copies)
            ...
            if self.offset_map:
                ...
                for one in section.variable:
                    ...
                    offset += len(one)
                body = records + copies + entries + relations + struct.pack(f"<{len(section.ids)}I", *section.ids)
                records_end = at + len(records) if section.records_end is None else section.records_end
                headers.append(
                    _SECTION.pack(
                        section.encryption_key,
                        at,
                        len(section.variable),
                        0,
                        records_end,
                        0,
                        len(relations),
        ...
        out = _HEADER.pack(
            ...
            len(self.fields),
            self.record_size,
            sum(len(s.strings) for s in self.sections),
            0,
            0,
            0,
            0,
        ...
        for stored in self.fields:
            out += _STORAGE_INFO.pack(
                stored.offset_bits, stored.size_bits, stored.additional, stored.storage, stored.default, 0, 0
            )
...
def test_a_pallet_value_masks_to_the_width_the_schema_declares() -> None:
    ...
    """
    table = Table(
        fields=[Stored(offset_bits=0, size_bits=5, storage=wdc3.PALLET, additional=8)],
...
def test_a_bitpacked_value_extends_over_its_own_width_when_that_is_narrower() -> None:
    ...
    """
    table = Table(
        fields=[Stored(offset_bits=0, size_bits=26, storage=wdc3.BITPACKED_SIGNED)],
...
def test_a_bitpacked_value_extends_over_the_declared_width_when_that_is_narrower() -> None:
    ...
    table = Table(
        fields=[Stored(offset_bits=0, size_bits=9, storage=wdc3.BITPACKED_SIGNED)],
...
def test_an_id_the_schema_puts_in_the_record_is_decoded_from_it() -> None:
    ...
    table = Table(
        fields=[Stored(offset_bits=0, size_bits=32), Stored(offset_bits=32, size_bits=32)],
        sections=[Built(records=struct.pack("<II", 900, 5) + struct.pack("<II", 901, 6))],
        record_size=8,
        has_id_list=False,
        schema=[ColumnSpec(name="FileDataID", is_id=True), ColumnSpec(name="Flags")],
    )
...
def test_a_copied_row_carries_its_new_id_in_the_id_column() -> None:
    ...
    table = Table(
        fields=[Stored(offset_bits=0, size_bits=32)],
        sections=[Built(records=struct.pack("<I", 11), ids=(1,), copies=((2, 1),))],
        record_size=4,
...
def test_a_section_holding_an_encryption_key_is_skipped() -> None:
    ...
    table = Table(
        fields=[Stored(offset_bits=0, size_bits=32)],
        sections=[
            Built(records=struct.pack("<I", 5), ids=(1,)),
            Built(records=struct.pack("<I", 6), ids=(2,), encryption_key=0xDEADBEEF),
        ],
        record_size=4,
...
def test_a_string_offset_is_measured_from_the_fields_own_position() -> None:
    ...
    table = Table(
        ...
        sections=[Built(records=struct.pack("<I", 4 + 1), strings=b"\0Parabola\0", ids=(3,))],
        record_size=4,
...
def test_a_string_offset_counts_records_the_section_does_not_hold() -> None:
    ...
    table = Table(
        fields=[Stored(offset_bits=0, size_bits=32)],
        sections=[Built(records=struct.pack("<I", 4 + 1 + 4), strings=b"\0Parabola\0", ids=(3,), unheld_records=1)],
        record_size=4,
        schema=[ID, ColumnSpec(name="Name", kind="string")],
    )
...
def test_an_array_expands_to_numbered_columns() -> None:
    ...
    assert read(table).columns == ["ID", "Attributes_0", "Attributes_1", "Attributes_2"]
...
def test_a_common_field_falls_back_to_its_declared_default() -> None:
    ...
    table = Table(
        fields=[Stored(offset_bits=0, size_bits=0, storage=wdc3.COMMON, additional=8, default=255)],
...
def test_a_relation_outside_the_record_comes_from_the_relationship_map() -> None:
    ...
    table = Table(
        fields=[Stored(offset_bits=0, size_bits=32)],
        sections=[Built(records=struct.pack("<I", 5) + struct.pack("<I", 6), ids=(1, 2), relations=((77, 0), (88, 1)))],
        record_size=4,
        schema=[ID, ColumnSpec(name="Value"), ColumnSpec(name="SpellID", is_relation=True, in_record=False)],
    )
...
def test_a_copy_rewrites_the_id_of_a_schema_less_row_not_its_first_value() -> None:
    ...
    table = Table(
        fields=[Stored(offset_bits=0, size_bits=32)],
        sections=[Built(records=struct.pack("<I", 7), ids=(1,), copies=((2, 1),))],
        record_size=4,
        schema=[],
    )

    assert list(wdc3.Db2(table.blob(), None).rows()) == [("1", "7"), ("2", "7")]
...
def test_an_offset_map_table_reads_records_the_map_locates() -> None:
    ...
    table = Table(
        fields=[Stored(offset_bits=0, size_bits=32), Stored(offset_bits=32, size_bits=32)],
        sections=[
            Built(variable=(b"Azeroth\0" + struct.pack("<i", 7), b"Kalimdor\0" + struct.pack("<i", 9)), ids=(1, 2))
        ],
        record_size=1,
        schema=[ID, ColumnSpec(name="Name", kind="string"), ColumnSpec(name="Value")],
        offset_map=True,
    )

    assert list(read(table).rows()) == [("1", "Azeroth", "7"), ("2", "Kalimdor", "9")]


def test_an_offset_map_record_holds_no_bytes_for_a_column_it_does_not_carry() -> None:
    ...
    table = Table(
        fields=[Stored(offset_bits=0, size_bits=32), Stored(offset_bits=32, size_bits=32)],
        sections=[Built(variable=(b"Azeroth\0" + struct.pack("<i", 7),), ids=(1,))],
        record_size=1,
        schema=[
            ID,
            ColumnSpec(name="Absent", in_record=False),
...
def test_an_offset_map_section_declaring_no_end_is_skipped() -> None:
    ...
    table = Table(
        fields=[Stored(offset_bits=0, size_bits=32)],
        sections=[Built(variable=(struct.pack("<i", 7),), ids=(1,), records_end=0)],
        record_size=1,
        schema=[ID, ColumnSpec(name="Value")],
        offset_map=True,
    )
...
def test_a_relationship_block_is_read_no_further_than_it_reaches() -> None:
    ...
    table = Table(
        fields=[Stored(offset_bits=0, size_bits=32)],
        sections=[Built(records=struct.pack("<I", 7), ids=(1,), relations=((42, 0),), declared_relations=99)],
        record_size=4,
        schema=[ID, ColumnSpec(name="Value"), ColumnSpec(name="Parent", is_relation=True, in_record=False)],
    )
...
def test_a_float_is_spelled_the_way_the_export_spells_it(value: float, text: str) -> None:
