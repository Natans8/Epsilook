...
def forms(displays: dict[int, list[int]] | None = None) -> ShapeshiftForms:
    ...
    return ShapeshiftForms(names={30: "Bear", 31: "Cat"}, displays=displays or {})
...
def test_a_creature_contributes_one_row_per_display_in_slot_order() -> None:
    ...
    assert [(row.subject, row.display, row.fid) for row in found.creatures] == [(1, 10, 900), (1, 11, 901)]
...
def test_a_summoned_creature_resolves_like_a_morphed_one() -> None:
    ...
    assert [(row.subject, row.display) for row in found.creatures] == [(1, 10), (1, 11), (2, 12)]
...
def test_a_display_named_in_two_slots_stays_two_rows() -> None:
    ...
    models = CreatureModels(displays={1: [(0, 10), (1, 10)]}, display_model={10: 100}, model_fid={100: 900})
    found = resolve_displays(morphing(1), models, forms())
