...
from pack.routes.sounds import Ambience, ZoneMusic, read_ambiences, read_zone_music
...
def test_a_music_set_is_its_name_and_its_day_and_night_kits(tables: BuildTables) -> None:
    assert read_zone_music(tables(ZoneMusic=ZONE_MUSIC))[2] == ZoneMusic("Zone-EvilForest", 2524, 2534)
...
def test_a_music_set_may_lack_one_of_its_kits(tables: BuildTables) -> None:
    ...
    assert read_zone_music(tables(ZoneMusic=ZONE_MUSIC))[3] == ZoneMusic("Trimmed", 0, 9)
...
def test_an_ambience_is_its_two_loops_and_nothing_else(tables: BuildTables) -> None:
    ...
    assert read_ambiences(tables(SoundAmbience=SOUND_AMBIENCE)) == {21: Ambience(4162, 4162), 22: Ambience(4163, 4204)}
