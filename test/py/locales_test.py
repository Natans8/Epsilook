...
from pack.derive import (CONTEXT_FIELDS, DEFAULT_LOCALE, LOCALES, SPOKEN_FIELDS,
                         DeriveContext, Spoken)
...
from pack.routes import (AreaGates, CreatureModels, GameObjectData, ItemModels,
                         MountData, ShapeshiftForms, SpellNames, SpellText)
...
from pack.tables.locales import (check_translation_declaration, locale_overlays,
                                 translated_exports)
...
def test_a_client_that_publishes_fewer_languages_names_the_ones_it_has() -> None:
    ...
    assert [locale.code for locale in locales_named(("ruRU",))] == \
           [DEFAULT_LOCALE, "ruRU"]
...
def test_every_field_a_language_replaces_is_read_by_some_section() -> None:
    ...
    sources for nothing.
    """
    read = {field for section in SECTIONS if section.localizable
            for field in section.reads}
...
def test_a_build_read_in_another_language_keeps_everything_else() -> None:
    ...
    said = Spoken(names=SpellNames(names={1: "Огненный шар"}), alt_names={},
                  templates=SpellText(), creatures=CreatureModels(),
                  items=ItemModels(), mounts=MountData(),
                  objects=GameObjectData(), forms=ShapeshiftForms(),
                  areas=AreaGates(), prose=context.prose)
...
def test_the_two_declarations_of_the_translation_overlay_agree() -> None:
    ...
    assert all(overlay.judged_on == LOCALE_COLUMN
               for overlay in locale_overlays("ruRU").values())
    assert set(locale_overlays("ruRU")) == {"creature_template", "gameobject_template"}
    assert not {overlay.table for overlay in locale_overlays("ruRU").values()} - set(
        TDB_LOCALE_TABLES["world"])
...
def test_a_translated_table_stands_in_for_the_build_s_own(build, translated) -> None:
    ...
    assert list(tables.rows("SpellName", ["ID", "Name_lang"])) == [
...
def test_the_build_decides_which_rows_the_table_has(build, translated) -> None:
    ...
    """
    (translated / "SpellName.csv").write_text(
        TRANSLATED + "12,Дуга,1\n", encoding="utf-8")
    tables = restated(build, translated, "SpellName")
    assert [row[0] for row in tables.rows("SpellName", ["ID", "Name_lang"])] == \
           ["10", "11"]
...
def test_a_column_the_caller_did_not_ask_the_key_for_still_joins(
    ...
    """
    tables = restated(build, translated, "SpellName")
    assert list(tables.rows("SpellName", ["Name_lang"])) == [
...
def test_a_patch_still_adds_the_rows_it_carries(build) -> None:
    ...
    (build / "creature_template_locale.csv").write_text(CREATURE_NAMES,
                                                        encoding="utf-8")
    world = CsvTables(build)
    patching = {"creature_template": replace(locale_overlays("ruRU")["creature_template"],
                                             restates=False)}
    tables = OverlaidTables(base=world, overlays=patching, source=world)
    assert [row[0] for row in tables.rows("creature_template", ["entry", "name"])] == \
           ["1", "2", "3"]
