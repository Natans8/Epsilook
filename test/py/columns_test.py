...
from pack.routes.columns import to_amount, to_int, to_int_from_float


@pytest.mark.parametrize(("text", "value"), [("7", 7), ("-1", -1), ("0", 0), ("", 0)])
def test_an_empty_cell_is_zero(text: str, value: int) -> None:
    """The game writes an empty cell where it means zero."""
...
def test_the_first_nonzero_spelling_wins() -> None:
    """Some builds export both spellings with the float left at zero, so
    "first nonempty" is not enough."""
...
def test_float32_noise_is_rounded_away_but_real_fractions_are_not() -> None:
    """The modern builds carry float32 conversion noise; 47.5 is not noise."""
