...
from pack.drift import (CREATURE_DISPLAY_SOURCES, OPTIONAL_COLUMNS, OPTIONAL_TABLES, SPELL_NAME_SOURCES,
                        TDB_CANDIDATE_TABLES, TDB_OPTIONAL_COLUMNS, TDB_OPTIONAL_TABLES)
...
def test_tdb_drift_names_tables_the_dumps_are_distilled_for() -> None:
    distilled = {table for roster in (TDB_TABLES, TDB_LOCALE_TABLES)
                 for kind in roster.values() for table in kind}
...
def test_a_candidate_table_is_one_a_reader_picks_between() -> None:
    ...
    assert not TDB_CANDIDATE_TABLES - (
        set(TDB_OPTIONAL_TABLES) | {table for kind in TDB_TABLES.values()
                                    for table in kind})
